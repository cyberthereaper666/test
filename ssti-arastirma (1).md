...
- Arithmetic sonucunun response içinde bulunması **tek başına** SSTI
  kanıtı sayılmaz; aşağıdaki kanıt/karşılaştırma kuralı hâlâ geçerlidir.
...
(`44435556`) marker'ın yerinde görünmeli **ve** (c) aynı payload'ın
...
eliminasyonunun ilk savunma hattıdır. Baseline/negative control ve
response diffing, mümkün olduğunda bağımsız bir evaluation evidence
(örn. Bölüm 12'deki error-based veya string-coercion probe) ile
desteklenmelidir.
...
2. **OOB (out-of-band):** Engine'in dosya/network erişimi varsa
   ...
   belirleyici değildir.** Bu nedenle blind confirmation'da önce
   timing, ancak izin varsa OOB tercih edilmelidir (OOB kanıtı çok
   daha güçlüdür, timing'in yanlış pozitif riski daha yüksektir); OOB
   denemesi yalnızca "sandbox yok" tespit edildiğinde değil, her
   durumda düşük riskli bir olasılık olarak değerlendirilmelidir.
...
**Sınıflandırma eşiği:**
- **Confirmed:** Marker + doğru hesaplanmış aritmetik sonuç (örn.
  `44435556`) + negatif kontrolün (encode edilmiş/kaçırılmış hali)
  farklı davrandığı + mümkünse ikinci bağımsız bir expression'ın (örn.
  `{{7*'7'}}` gibi string testi) de tutarlı sonuç verdiği durum. Bu
  ikinci expression **aynı temel evaluation mekanizmasına** dayanıyorsa
  (örn. sadece delimiter'ı değiştirilmiş aynı aritmetik işlem) bağımsız
  bir kanıt olarak sayılmaz — gerçekten farklı bir evidence kategorisi
  (error-based, string-coercion, boolean) olmalıdır.
