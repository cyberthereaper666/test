...
**Sınıflandırma eşiği — önemli ilke:** "Confirmed" durumu **tek bir
zorunlu kombinasyon** (marker + arithmetic + negative control) olarak
tanımlanmamalıdır. Bu üçlü **yeterli bir kombinasyondur**, ama **tek
geçerli yol değildir**. Doğru tanım şudur: **Confirmed = yeterli ve
birbirinden bağımsız server-side evaluation evidence'ı** — bu evidence
farklı kombinasyonlardan gelebilir. Örneğin güçlü bir template compile
error (engine adını doğrudan veren bir stack trace) + engine-specific
bir evaluation davranışı + bağımsız bir confirmation, arithmetic probe
hiç kullanılmadan da "confirmed" için yeterli olabilir — özellikle
...
- Marker + doğru hesaplanmış aritmetik sonuç (örn. `44435556`) +
  ...
  string-coercion testi) de tutarlı sonuç verdiği durum.
